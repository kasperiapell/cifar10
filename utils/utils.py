...
DATA_PATH = '/Users/shared/data/CIFAR-10/train.csv'

def get_raw_data():
	data_raw = genfromtxt(DATA_PATH, delimiter = ',', dtype = np.float32)
	data = data_raw[1:,]

	feature_data_raw = data[0:,:-1]
	label_data_raw = data[:,-1]
...
def get_data_loader(features_train, 
					...
					targets_test, 
					batch_size = 256):
	rs_features_train = features_train.view(-1, 3, 32, 32).float()
	rs_features_test = features_test.view(-1, 3, 32, 32).float()
